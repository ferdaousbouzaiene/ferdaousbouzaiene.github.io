...
### 📚 [Source Q&A Chatbot with RAG](https://github.com/ferdaousbouzaiene/RagProject)
[![Live Demo](https://img.shields.io/badge/Live-Demo-brightgreen)](url)
[![GitHub](https://img.shields.io/badge/GitHub-Repository-black)](https://github.com/ferdaousbouzaiene/RagProject)

Built a RAG pipeline that enables users to upload documents or provide URLs, then query them with grounded answers generated by LLMs.

- Tools: FastAPI, OpenAI API, ChromaDB, Langchain, Streamlit  
- Highlights:
    - 📄 Supports multi-format document ingestion (PDF, TXT, DOCX)

    - 🔗 Handles web URLs for dynamic knowledge ingestion

    - ✂️ Implements chunking & embedding strategies for long docs

    - 🔍 Uses vector search (Chroma) for fast, relevant retrieval

    - 🧠 Generates LLM answers with source citations

    🎨 Includes a lightweight Streamlit UI for uploads, queries, and results

**Impact**: Strengthened expertise in RAG patterns, vector search, and LLM integration, while building a practical, production-ready knowledge assistant.  
